<section class="content-header">
  <h1>Aksesoris Keluar
    <small>Aksesoris Keluar</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href=""></i class="fa fa-dashboard"></i></a></li>
    <li class="active">Aksesoris Keluar</li>
  </ol>
</section>
...
<script>
  ...
  $(document).on('click','#add_cart', function() {
    ...
    if(id_item_acc == ''){
      ...
      $.ajax({
        type : 'POST',
        url  : '<?=site_url('Aksesorisout/process')?>',
        data : {'add_tempout':true, 'id_item_acc':id_item_acc, 'qty':qty, 'supplier':supplier, 'ref':ref},
        dataType: 'json',
        success: function(result) {
          if(result.success == true){
            $('#cart_table').load('<?=site_url('Aksesorisout/temp_data')?>', function(){
  ...
  $(document).on('click','#del_temp', function(){
    if(confirm('Apakah anda yakin')){
      ...
      $.ajax({
        type:'POST',
        url: '<?=site_url('Aksesorisout/del_tempout')?>',
        dataType: 'json',
        data:{'id_temp_accout':id_temp},
        success: function(result){
          if(result.success == true){
            $('#cart_table').load('<?=site_url('Aksesorisout/temp_data')?>', function(){
            }) 
    ...
    var supplier    = $('#supplier').val()
       if(confirm(' Yakin proses transaksi ini ?')){
         $.ajax({
           type : 'POST',
           url  : '<?=site_url('Aksesorisout/process') ?>',
           data : {'process_saving' : true, 'date':date, 'sj':sj, 'supplier':supplier},
           dataType : 'json',
           success  : function (result) {
              if(result.success){
              ...
              location.href='<?=site_url('Aksesorisout')?>'
             }
         })
       }
    })
