      ...
      <div class="box box-widget">
        <div class="box-body table-responsive">
          <table class="table table-bordered table-striped">
            ...
            <tbody id="cart_table">
              <?php $this->view('transaction/barangmasuk/temp_barangmasuk') ?>
            </tbody>
          </table>
        </div>
      </div>
...
<script>
  ...
  $(document).on('click','#add_cart', function() {
    ...
    if(id_item == ''){
      ...
      $.ajax({
        type : 'POST',
        url  : '<?=site_url('Barangmasuk/process')?>',
        data : {'add_temp':true, 'id_item':id_item, 'qty':qty, 'supplier':supplier, 'ref':ref},
        dataType: 'json',
        success: function(result) {
          if(result.success == true){
            $('#cart_table').load('<?=site_url('barangmasuk/temp_data')?>', function(){
  ...
  $(document).on('click','#del_temp', function(){
    if(confirm('Apakah anda yakin')){
      ...
      $.ajax({
        type:'POST',
        url: '<?=site_url('Barangmasuk/del_temp')?>',
        dataType: 'json',
    ...
    var supplier    = $('#supplier').val()
       if(confirm(' Yakin proses transaksi ini ?')){
         $.ajax({
           type : 'POST',
           url  : '<?=site_url('Barangmasuk/process') ?>',
           data : {'process_saving' : true, 'date':date, 'sj':sj, 'no_po':no_po, 'supplier':supplier},
           dataType : 'json',
           success  : function (result) {
              if(result.success){
              ...
              location.href='<?=site_url('Barangmasuk')?>'
             }
         })
       }
    })
